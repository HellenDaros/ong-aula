...
export async function alterarStatusAnimal(animal: Animal): Promise<boolean> {
    try {
        ...
        const response = await api.put(`/animais/${animal.id}/AlterarStatusAnimal`, payload);
...
export async function salvarAnimal(animal: Animal, isEdicao: boolean): Promise<boolean> {
    try {
        let response;
        if (isEdicao) {
